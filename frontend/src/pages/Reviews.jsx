...
function ReviewModal({ employees, onClose, onSaved }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-[#0d1117] border border-white/10 rounded-2xl w-full max-w-lg shadow-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-white/5 sticky top-0 bg-[#0d1117]">
          <h2 className="text-white font-semibold text-lg">New Performance Review</h2>
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-medium text-gray-400 mb-1.5">Review Period</label>
            ...
            <div className="flex gap-2 items-center">
              {[1,2,3,4,5].map(n => (
                <button key={n} onClick={() => setForm({...form, overallRating: n})}
                  className={`w-10 h-10 rounded-xl text-sm font-bold border-2 transition-all ${form.overallRating === n ? 'border-indigo-500 bg-indigo-500/20 text-indigo-300' : 'border-white/10 bg-white/5 text-gray-500 hover:border-white/20'}`}>
                  {n}
                </button>
              ))}
          ...
          <div>
            <label className="block text-xs font-medium text-gray-400 mb-1.5">Strengths</label>
          ...
          <div>
            <label className="block text-xs font-medium text-gray-400 mb-1.5">Areas of Improvement</label>
          ...
          <div>
            <label className="block text-xs font-medium text-gray-400 mb-1.5">Comments</label>
    ...
    <div className="p-6 max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-0.5">Periodic performance evaluations</p>
        </div>
        {isManager() && (
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {reviews.map(rv => (
            <div key={rv._id} className="bg-[#0d1117] border border-white/5 rounded-2xl overflow-hidden hover:border-white/10 transition-all">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-full bg-indigo-600 flex items-center justify-center flex-shrink-0">
                      <span className="text-white text-sm font-bold">{rv.employee?.name?.[0]?.toUpperCase()}</span>
                    </div>
                    <div>
                      <div className="text-white font-semibold text-sm">{rv.employee?.name}</div>
              ...
              {expanded === rv._id && (
                <div className="px-5 pb-5 border-t border-white/5 pt-4 grid gap-3">
                  {rv.strengths && (
